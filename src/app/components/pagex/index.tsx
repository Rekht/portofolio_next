...
export const Router = ({ component, children }: RouterProps) => {
  ...
  // App Router handles route changes differently - through pathname changes
  useEffect(() => {
    if (match?.route.path) {
      manager.dispatch({
        type: "ROUTE_CHANGE",
  ...
  }, [match?.route.path, component, pathname]); // Add pathname to dependencies
...
export type KeyframesObj = Record<string, StyleObj>;
